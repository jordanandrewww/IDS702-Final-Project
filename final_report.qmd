...
ggplot(music_data_1_subset,
  ...
  labs(
    title = "Figure 1: Daily Listening Time vs. Depression Score by Streaming Platform",
    x = "Depression Score",
    y = "Hours per Day",
...
kable(tbl_rq1_sig,
      caption = "Table 1: Significant Predictors from RQ1 Linear Model",
      booktabs = TRUE) %>%
...
kable(
  tbl_rq2_condensed,
  caption = "Table 2: Key Predictors from Logistic Regression (Odds Ratios)",
  booktabs = TRUE
) %>%
  kable_styling(full_width = FALSE)
```
...
```{r, fig.cap="Figure 2: Diagnostic plots for the raw RQ1 model.", fig.align='center'}
par(mfrow = c(2, 2))
plot(model_rq1_raw)
par(mfrow = c(1, 1))
```
