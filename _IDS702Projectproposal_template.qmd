...
This dataset is a large flight-pricing dataset scraped from the online travel agency EaseMyTrip.com. The data was collected between February 11 and March 31st of 2022, using the Octoparse web-scraping tool and contains 300,153 unique flight booking options for travel between India's 6 major metropolitan cities. Each row represents 1 flight option. The data consists of both economy and business-class listings, which were collected separately and then merged into a single cleaned dataset. The dataset contains 11 columns, including airline, source city, class, flight duration, and price.

**Research question 1:**

How does the average ticket price differ across airlines, and does this relationship depend on seat class (Economy vs Business), controlling for route characteristics?

Outcome variable: ticket price (continuous)

**Research question 2:**

Do business-class flights tend to depart at systematically earlier or later times of day compared to economy flights?

Outcome variable: Arrival time of day ordinal/categorical (5 categories)
...
flight_data <- read.csv("data/airline_data.csv")
glimpse(flight_data)
```
...
ggplot(flight_data, aes(x = price)) +
  ...
  labs(
    title = "Histogram of Ticket Prices",
    x = "Ticket Price (INR)",
    y = "Count"
  ) +
  theme_minimal()

# Ticket price by airline and seat class
ggplot(flight_data, aes(x = airline, y = price, fill = class)) +
  geom_boxplot(position = position_dodge()) +
  labs(
    title = "Ticket Price by Airline and Seat Class",
    x = "Airline",
    y = "Ticket Price (INR)",
    fill = "Seat Class"
  ) +
...
```{r}
# Distribution of arrival time categories
ggplot(flight_data, aes(x = arrival_time)) +
  geom_bar() +
  labs(
    title = "Distribution of Arrival Time Categories",
    x = "Arrival Time of Day",
    y = "Count"
  ) +
  theme_minimal()

# Arrival time distribution by seat class
ggplot(flight_data, aes(x = arrival_time, fill = class)) +
  geom_bar(position = "dodge") +
  labs(
    title = "Arrival Time Category by Seat Class",
    x = "Arrival Time of Day",
    y = "Count",
    fill = "Seat Class"
  ) +
...
# Notice that there are a lot more economy flights in this dataset so might have to even out that distribution
